...
paCalculation <- function(N, s, pp=9/11, ps=0.75, beta=1) {
  ...
  if(pp == 1) {
    ...
    print(paste0("No pa satisfies E(fb|S,N-S,ps,pa)=Fb. Fb needs to be less than ", round(test1(0), 2),
                ". Force pa as 0."))
    pa <- 0; details <- NULL
  } else if(test1(1) > Fb) {
    print(paste0("No pa satisfies E(fb|S,N-S,ps,pa)=Fb. Fb needs to be larger than ", round(test1(1), 2),
                ". Force pa as 1."))
    pa <- 1; details <- NULL
  } else {
