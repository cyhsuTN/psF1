...
#' @examples #            beta=1,
#' @examples #            alternative=c("two.sided", "less", "greater"),
#' @examples #            normal.approx=c("auto", "TRUE", "FALSE"),
  ...
  if(is.null(pp1) & is.null(ps1)) {
    ...
    allf1 <- lapply(xx, function(x) {
      ...
      res <- try({
        ...
        if(normal.approx=="auto" & length(f1s) > 1E+4) normal.approx <- "TRUE"
    ...
    yy <- sapply(xx, function(x) {
      ...
      res <- try({
        ...
        pw.s <- sapply(allf1, function(u) {
          ...
          if(normal.approx=="auto" & (length(f1s) > 1E+4 | length(f1s1) > 1E+4)) normal.approx <- "TRUE"
  ...
  if(is.null(pp) & is.null(ps)) {
    ...
    yy <- sapply(xx, function(x) {
      ...
      res <- try({
        ...
        if(normal.approx=="auto" & (length(f1s) > 1E+5 | length(f1s1) > 1E+5)) normal.approx <- "TRUE"
